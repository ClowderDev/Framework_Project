...
namespace Framework_Project.Controllers
{
    public class HomeController : Controller
    {
        ...
        {
            var user = await _userManager.GetUserAsync(User);
            var compareCount = await _dataContext.Compares.Where(c => c.UserId == user.Id).CountAsync();
            if (compareCount >= 3)
            {
                return BadRequest(new { success = false, message = "You can only add up to 3 products to compare." });
            }
